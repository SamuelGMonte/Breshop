$(document).ready(function() {
    ...
    let vendedorId = documentLocation.match(/(?<=\/vendedor\/)\d+(?=\/brecho)/);
    console.log(vendedorId[0]);
    $.when(
        $.ajax({ url: `/api/v1/brecho/${vendedorId[0]}/meus-brechos` }),   
        $.ajax({ url: `/api/v1/brecho/${vendedorId[0]}/enderecos` }),   
        $.ajax({ url: `/api/v1/brecho/${vendedorId[0]}/site` }),   
        $.ajax({ url: `/api/v1/brecho/${vendedorId[0]}/imagens` }),   
